...
score <- function(text){
  token <- limpiar_texto(text)
  v <- 0
  for(word in token){
    if(word %in% lexico$word){
      v <- v + lexico[lexico$word == word,]$value
...
```
```{r}
tweets <- tweets %>% mutate(Score = map(.x = OriginalTweet, .f = score))
```

```{r}
to_score <- function(word){
  if(word == "Positive" | word=="Extremely Positive"){
    return(1)
  }
  if(word == "Negative" | word=="Extremely Negative"){
    return(-1)
  }
  else{
    return(0)
...
tweets <- tweets %>% mutate(Score_real = map(.x = Sentiment, .f = to_score))
```


```{r}
tweets$bin_error <-ifelse((tweets$Score>0 & tweets$Score_real>0)| (tweets$Score<0 & tweets$Score_real<0),1,0)
```

```{r}
sum(tweets$bin_error)*100/nrow(tweets)
```
...
covid <- covid[covid$date >= min(df$TweetAt) & covid$date <= max(df$TweetAt) & covid$location == "World",]
covid_new_cases <- covid %>% select(date, new_cases)


covid_nc <- covid_new_cases %>% mutate(mes_dia = format(date, "%m-%d")) %>% select(-c(1))
tw_mes_dia <- tweets_mes_dia %>% group_by(Sentiment, mes_dia) %>% summarise(n = n())
tw <- merge(tw_mes_dia, covid_nc, by = "mes_dia")
