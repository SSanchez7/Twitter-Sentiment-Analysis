...
La metodologia consiste principalmente en los siguientes pasos (algunos de ellos, ya han sido realizados, por lo que)
...
* Se removeran notaciones propias de Twitter como los Hashtag (#), retweets (RT) e identificadores de cuenta (@). *REVISAR*
* Se eliminaran URL's, hipervinculos, emoticones y todo dato y simbolo que no sean letras, para si tratar solamente con datos de texto. *REVISAR*
* Se mejorara el filtro de palabras "vacias" del lenguaje (conectores, etc), que no enfatizan ninguna emocion y extienden innecesariamente el conjunto ed datos para el estudio. *REVISAR*
* Se comprimiran palabras alargadas (holaaaa -> hola) y descomprimiran las abreviadas. Estas ultimas, por lo general son adjetivos o sustantivos que contienen mucho sentimiento, importante para el analisis. *REVISAR*
  
2. Extraccion de caracteristicas importantes del texto limpio.
* Se extraeran características del texto de los tweets (como adjetivos, verbos y sustantivos) y representándolas en forma vectorial se les asignara un sentimiento, para asi lograr detectar la polaridad de la oracion completa, enfocandose en terminos de freceuncia y precensia de estas palabras. Esta seccion se realizó en parte en el Hito 1, pero para el Hito 3 se trabajará de forma más sofisticada. *REVISAR*
...
* Gracias a que los datos ya venian etiquetados con valores de sentimiento, no se hace necesario etiquetar de forma manual el dataset. NO bien, reduciremos el numero de clases de la clasificacion dada (consiste de 5), agrupando las "Extremadamente negativos" y "Extremadamente positivos" en "Negativo" y "Positivo" respectivamente, esto para lograr reducir ... *REVISAR*
* Para definir el modelo que arroja una mejor clasificación se evaluará la base de datos usando Naive Bayes, Arboles de desicion, K_Mean y SVM, en donde se utilizan las columnas por palabras creadas anteriormente para poder predecir un sentimiento y además, para el caso de K_Mean se utilizará el método del codo para definir la cantidad de clusters. Se asignaran hiperparámetros de evaluación a cada uno de los modelos a usar utilizando técnicas como GridS.
