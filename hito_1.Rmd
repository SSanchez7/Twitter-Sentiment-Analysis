...
Esta pregunta fue respondida para la base de datos inicial en el Hito 1 sin embargo se incorporará nueva información que necesita ser evaluada (set de evaluación).  Se comparán los análisis de sentimiento obtenidos para los dos grupos de información para observar si hubo alguna variación de sentimiento en estos dos instantes de tiempo.
...
Debido a la orientación que se le dará finalmente al proyecto esta pregunta se encuentra fuera de los alcances, ya que mas del 20% de las ubicaciones están omitidas, por lo tanto, se eliminará. 

4.	¿Se puede asociar un sentimiento a una palabra dependiendo de las otras palabras que forman el tweet analizado?

No se evaluará debido a la complejidad de las herramientas necesarias para abordar el problema, estas no se encuentran en el contenido del curso, ya que requiere técnicas mas sofisticadas de procesamiento de lenguaje natural.

5.	¿Existe una variacion de comentarios positivos al avanzar los días? ¿Si es así, a qué se debe?

No es posible establecer una razón clara a la cual atribuir la variación de sentimiento en los tweets, ya que existe cierta intermitencia en la cantidad de tweets por día, por lo que no se pueden generalizar tendencias, sólo observar y tener una idea a grandes rasgos.
...
Con el fin de responder las preguntas e hipótesis que quedan por evaluar, se propone seguir la siguiente metodología.

La metodologia consiste principalmente en los siguientes pasos (algunos de ellos, ya han sido realizados)
...
* Se removeran notaciones propias de Twitter como los Hashtag (#), retweets (RT) e identificadores de cuenta (@). 
* Se eliminaran URL's, hipervinculos, emoticones y todo dato y simbolo que no sean letras, para si tratar solamente con datos de texto. 
* Se mejorara el filtro de palabras "vacias" del lenguaje (conectores, etc), que no enfatizan ninguna emocion y extienden innecesariamente el conjunto de datos para el estudio. 
* Se comprimirán palabras alargadas (holaaaa -> hola) y descomprimiran las abreviadas. Estas ultimas, por lo general son adjetivos o sustantivos que contienen mucho sentimiento, importante para el analisis. 
  
2. Extraccion de caracteristicas importantes del texto limpio.
* Se extraerán características del texto de los tweets (como adjetivos, verbos y sustantivos) y representándolas en forma vectorial se les asignara un sentimiento, para asi lograr detectar la polaridad de la oracion completa, enfocandose en terminos de freceuncia y presencia de estas palabras. Esta seccion se realizó en parte en el Hito 1, pero para el Hito 3 se trabajará de forma más sofisticada. 

3. Entrenamiento del modelo.
* Se dividirá la base de datos en dos grupos: set de entrenamiento y set de evaluación, con un porcentaje de 70% y 30% respectivamente .
* Gracias a que los datos ya venian etiquetados con valores de sentimiento, no se hace necesario etiquetar de forma manual el dataset. No bien, reduciremos el numero de clases de la clasificacion dada (consiste de 5), agrupando las "Extremadamente negativos" y "Extremadamente positivos" en "Negativo" y "Positivo" respectivamente, esto para lograr definir un clasificador sólo en base a estas clases.
* Para definir el modelo que arroja una mejor clasificación se evaluará la base de datos usando Naive Bayes, Arboles de desicion, K_Mean y SVM, en donde se utilizan las columnas por palabras creadas anteriormente para poder predecir un sentimiento y además, para el caso de K_Mean se utilizará el método del codo para definir la cantidad de clusters. Se asignaran hiperparámetros de evaluación a cada uno de los modelos a usar utilizando técnicas como GridSearch.

4. Evaluacion del modelo
* Los resultados obtenidos al aplicar los diferentes modelos se compararán en base a métricas tradicionales como F1, precision y recall, utilizando una matriz de confusión para cada caso. Además se aplicará validación cruzada sobre los grupos de entrenamiento  y testeo respectivamente. 
* Después de elegir el modelo que mejor clasifica los datos a partir de las métricas mencionadas, se usará ese modelo para asignar sentimiento a una base de datos nueva que será limpiada y estandarizada según los atributos de la base de datos inicial.

5. Extension a nuevo dataset.
* Se agrupará la asignación de sentimiento de la base de datos nueva según ubicación y se comparará con la base de datos inicial, para así identificar si existe alguna tendencia o diferencia en dos periodos de tiempo. Uno al inicio de la pandemia y el otro ya transcurrido algunos meses.
...
1. Nicolás Herrera: Redacción de metodología, análisis de gráficos y conclusiones.

2. Yesenia Marulanda: Redacción de objetivos, metodología y respuesta a preguntas planteadas.

3. Franco Migliorelli: Creación de código para separación de columnas por sub-localidades.

4. Samuel Sánchez: Planteamiento de las bases de la metodología, redacción y análisis.

5. Sebastián Urbina: Creación de gráficos, limpieza, tratamientos de datos y cruzamiento con casos de Coronavirus.
